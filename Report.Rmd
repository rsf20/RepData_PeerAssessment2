...
```
## Human Harm
There are `r nrow(filter(harmful_totals, fatalities != 0))`that caused deaths and `r nrow(filter(harmful_totals, injuries != 0))` that caused injuries. These are plotted here on logarithmic scales (TORNADO is much bigger than the others) and then listed sorted descending first by deaths than by injured:
```{r}
...
ggplot(humans_harmed, aes(fatalities, injuries)) + 
    geom_text(aes(label = event_type)) + scale_x_log10() + scale_y_log10()
print(humans_harmed)
```
...
ggplot(cash_dmg, aes(propdmg, cropdmg)) + 
    geom_text(aes(label = event_type))
print(cash_dmg)
```
Clearly the first six on the list have the biggest impact on property and crops.
